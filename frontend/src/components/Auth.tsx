...
export default function Auth({ onLoginSuccess }: AuthProps) {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auto populate login fields and switch to login
      setPassword('');
      setConfirmPassword('');
      setSecurityAnswer('');
      setTimeout(() => {
      ...
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-xl p-8 shadow-2xl z-10 transition-all duration-300 relative overflow-hidden" id="auth-card">
        ...
        {mode === 'login' && (
          <form onSubmit={handleLogin} className="space-y-5" id="login-form">
            ...
            {/* Quick Demo Admin Credentials Helper - Optional: keep for dev ease but make more generic */}
            <div className="p-3 bg-slate-950 border border-slate-800 rounded-xl space-y-1.5">
              <div className="flex items-center justify-between text-[11px] font-bold text-slate-300">
                <span className="flex items-center gap-1.5 text-indigo-400">
                  <Sparkles className="w-3.5 h-3.5" />
                  <span>Workspace Admin Account</span>
                </span>
                <button
                  type="button"
                  onClick={() => {
                    setEmail('admin@pinobite.com');
                    setPassword('Password123!');
                  }}
                  className="px-2 py-0.5 bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-300 rounded font-semibold text-[10px] transition-colors cursor-pointer"
                >
                  Auto-fill Admin
                </button>
              </div>
              <div className="text-[11px] font-mono text-slate-400 space-y-0.5">
                <div>Email: <span className="text-white">admin@pinobite.com</span></div>
                <div>Password: <span className="text-amber-400 font-bold">Password123!</span></div>
              </div>
            </div>
        ...
        {mode === 'signup' && (
          <form onSubmit={handleSignup} className="space-y-4" id="signup-form">
            <div className="space-y-1">
              <h2 className="text-xl font-bold text-white">Create Workspace Account</h2>
              <p className="text-xs text-slate-400">Set up your credentials and recovery verification.</p>
            </div>
            ...
            <div className="space-y-3 max-h-[380px] overflow-y-auto pr-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-medium text-slate-400 block">Confirm</label>
                  <input
                    type="password"
                    required
                    placeholder="••••••••"
              ...
              {/* Secure Account Recovery Config */}
              <div className="p-3 bg-slate-950 border border-slate-800 rounded-lg space-y-2.5">
                <div className="flex items-center gap-1.5 text-indigo-400 text-xs font-bold">
                  ...
                  <span>Secure Account Recovery Setup</span>
                </div>
                <p className="text-[10px] text-slate-500">Used to securely reset your password without email delivery dependancies in this environment.</p>

                <div className="space-y-1">
                  <label className="text-[10px] font-medium text-slate-400">Security Question</label>
                  <select
                    value={securityQuestion}
                    onChange={(e) => setSecurityQuestion(e.target.value)}
                    className="w-full bg-slate-900 text-slate-300 border border-slate-800 focus:border-indigo-500 rounded-lg py-1 px-2 text-xs outline-none cursor-pointer"
                  >
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-medium text-slate-400">Your Answer (not case sensitive)</label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full bg-slate-900 text-slate-200 border border-slate-800 focus:border-indigo-500 rounded-lg py-1 px-2.5 text-xs placeholder-slate-600 outline-none"
                  />
                </div>
              </div>
            </div>
            ...
            <button
              type="submit"
              className="w-full bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 text-white font-medium text-sm py-2 rounded-lg flex items-center justify-center gap-2 shadow-lg shadow-indigo-950/30 transition-all cursor-pointer"
